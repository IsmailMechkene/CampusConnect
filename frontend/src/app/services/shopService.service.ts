...
export class ShopService {
  ...
  /** Récupérer un shop par son ID depuis le backend */
  getShopByIdHttp(id: string): Observable<Shop | null> {
    return this.http.get<Shop>(`${this.baseUrl}/shop/${id}`).pipe(
      catchError((err) => {
        console.warn('Failed to fetch shop by id', err);
        return of(null);
      })
    );
  }
  ...
  private mockShops: Shop[] = [
    {
      ...
      description:
        'Nulla sagittis ex sed tellus porttitor, vel dignissim magna euismod. Maecenas sit amet malesuada lorem, ac ornare erat.',
  ...
  private mockShopDetails: ShopDetails[] = [
    {
      ...
      description:
        'Spark agency specializes in homemade bracelets suitable for every taste. A broad selection of different bracelets designs',
